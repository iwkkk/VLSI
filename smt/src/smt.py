...
def solve_instance(data):
    ...
    #symmetry breaking
    symmetry_breaking = [And(y[height.index(max(height))] == 0, x[height.index(max(height))] == 0)]
    ...
    constraints =domain_x_l + domain_y_l + width_constraint + height_constraint + \
        noOverlap + cumulative_x + cumulative_y + symmetry_breaking + symmetry_breaking_same
...
def plot_result(instance,Xposition,Yposition,width,height,w,n,h):
    ...
    plt.plot()
    plt.savefig('./smt/output/noRotation/{0}.jpg'.format(instance))
    plt.close(fig)
    #plt.show()

def write_file(instance,Xposition,Yposition,width,height,w,n,h,duration):
        with open('./smt/output/noRotation/{0}.txt'.format(instance),'w') as f:  
            f.writelines('%d %d \n' %(w, h))
            f.writelines(str(n)+'\n')
            for i in range(n):
...
def main():
    ...
    runingData.to_csv('./smt/output/noRotation/1.csv') 
    # data = read_file(1)
    # solve_instance(data)
