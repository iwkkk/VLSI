...
def solve_instance(data):
    ...
    constraints = domain_x_l + domain_y_l + width_constraint + height_constraint + noOverlap + \
        cumulative_x + cumulative_y + symmetry_breaking + symmetry_breaking_same
...
def plot_result(instance,x,y,width,height,w,n,h,rotation):
    ...
    plt.plot()
    plt.savefig('./smt/output/rotation/{0}.jpg'.format(instance))
    plt.close(fig)
    #plt.show()
...
def main():
    ...
    runingData.to_csv('./smt/output/rotation/1.csv') 
