...
modelPath = "./cp/src/cp_normal.mzn"
w = 0  # width
n = 0  # number
...
for file in files:
    ...
    if result.statistics['solutions']== 0:
        ...
        with open('./cp/output/1/out-{0}.txt'.format(fileIndex),'w') as f:  
            f.writelines('%d %d \n' %(w,result["objective"]))
            f.writelines(str(n)+'\n')
            for i in range(n):
        ...
        plt.plot()
        plt.savefig('./cp/output/1/pic-{0}.jpg'.format(fileIndex))
        plt.close(fig)
        #plt.show()
...
runingData.to_csv('./cp/output/1/result.csv') 
